...
export interface SolicitacaoProtheus {
  ...
  solicitante?: string; // TQB_PRIORI.
}

export interface SolicitacaoProtheusOS {
  numeroSolicitacao?: string; // TQB_SOLIC - Não usamos, gerado automaticamente pelo Protheus.
  origin: string; // TQB_ORIGEM - Obrigatório.
  equipment: string; // TQB_CODBEM - Obrigatório.
  costCenter: string; // TQB_DESCRICAO - Obrigatório.
  startDate?: string; // TQB_CDSERV - Condicional.
  service?: string; // Condicional
  situation?: string; // TQB_PRIORI.
  inputs?: []; // TQB_PRIORI.
}
