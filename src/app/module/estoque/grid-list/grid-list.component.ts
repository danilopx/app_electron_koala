...
import {
  ...
  PoModalComponent, PoModalAction, PoNotificationService, PoPageFilter, PoPageAction, PoMenuItem
...
export class GridListComponent implements OnInit, OnDestroy {
  ...
  readonly menus: Array<PoMenuItem> = [
    { label: 'Painel de Controle', link: '../producao', shortLabel: 'Painel de Controle', icon: 'po-icon po-icon-manufacture' },

    { label: 'estoque', link: '../estoque', icon: 'po-icon po-icon-sale', shortLabel: 'estoque' },
    {
  ...
  sortBy(event: { column: { property: string; }; type: string; }) {
    this.offset = 1;
  ...
  private onEditGrid(record: { id: any; }) {
  ...
  private onRemoveGrid(record: { id: string; }) {
  ...
  private onViewGrid(record: { id: any; }) {
